...
> - **Tier 2:** 2.1 (reviewCleared latch removed, `65615ae`), 2.2 + 2.3 (onboarding bricks) + 2.4 (Apple deletion hole) fixed `8718cf5`, 2.5 (sentence SAY result, `359d91a`), 2.9 + 2.10 (dictation play-budget + accent caret, `2ffb546`), 2.11 (hydration race — `_layout` gates all three stores), 2.12 ◐ (errors now ring-buffered + fallback screen; remote reporter a deliberate Phase-3 stub), 2.13 ◐ (cleanup added to flashcards/voiceflash/dictation; player still pending, Phase 5).
...
> - **Facade screens now honest:** 1.4 (player → real listening pass, `ed64266`), 1.10 (downloads → offline/content status, `4d25094`), 1.11 (chat → real greeting/timestamps/online-state, `1dbbe49`).
> - **Remaining Tier-1:** only 1.7 (settings Visa/Restore) — deferred to the monetization review.
...
| 1.19 | **The Ops Console has no 2FA, and a dead env var is disguising that fact.** `ealch-admin/.env` carries `DISABLE_TOTP=true` with the comment *"TEMP: skip TOTP verification at login while testing. Delete this line to re-enable 2FA."* **No source file reads `DISABLE_TOTP`.** It is a no-op; deleting it would change nothing. The real switch is a **database column**: `auth.ts:45` enforces TOTP only `if (admin.totpSecret)`. Queried against production, the sole admin — **`freetinz@gmail.com`, `super_admin`** — has `totp_secret = NULL`, so **2FA is off**. That account's console holds a raw Postgres connection to production that **bypasses RLS and has full DDL/DML** (more privileged than `service_role`), protected by email and password alone. The comment is the hazard: it makes the missing 2FA read as deliberate, temporary and env-controlled, when it is a permanent DB state. **Fix:** set a `totp_secret` for that admin (`scripts/seed.ts:66-75` already generates `otpauth://` QR URLs; `otplib` and `qrcode` are installed) and delete the misleading line. Do it before the console leaves localhost. |
