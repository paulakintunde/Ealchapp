// The Den — content_units filtered to kind='lesson'. Detail editing happens
// at /admin/content/[id], which renders the LessonBodyEditor block library
// + narration markup editor for this kind.
import Link from 'next/link';
import { desc, eq } from 'drizzle-orm';
import { db, schema } from '@/db';
import { STATUS_META, chipStyle } from '../meta';
import { relTime } from '@/lib/format';
import NewDenLessonButton from './NewDenLessonButton';
      ...
      <section className={styles.card}>
        <div className={styles.cardHead} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div>
            ...
            <div className={styles.cardSub}>Guided narrated lessons — sections, practice items, and Camille&apos;s script</div>
          </div>
          <div style={{ flex: 1 }} />
          <NewDenLessonButton />
        </div>
        ...
        {rows.length === 0 ? (
          <div className={styles.empty}>No Den lessons yet — create one to get started.</div>
        ) : (
          rows.map((r) => (
