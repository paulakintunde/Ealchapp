// a2.29.l1 « À l'hôtel » — merge into ealch-v2/src/content/seed.json.
//
//   pnpm tsx scripts/merge-hotel-into-seed.ts
//
// THE SEED IS A CUT, AND THIS UNIT SITS ON THE WIDEST DIVERGENCE IN THE BAND
// AFTER a2.28's. Before this merge, `hebergement` holds 371 published rows in
// Postgres and ZERO in the seed. So this script does NOT merely add the
// authored rows. It pulls every row the lesson REFERENCES out of Postgres and
// writes those too, or the imported cards render empty on device while the
// tests pass against a corpus that has them.
//
// THAT DIVERGENCE IS THE BUILD'S HEADLINE. Anyone measuring this unit against
// `seed.json` would conclude the hotel corpus does not exist and author a
// hundred duplicate rows. It exists and it is finished: this build authored
// ZERO hotel nouns and imported thirteen of them.
//
// THIS MERGE CARRIES ROWS INTO THEMES THIS UNIT DOES NOT OWN: `au-restaurant`
// (a2.07's frozen repair block), `bricolage`, `expressions-frequentes`,
// `salutations`, `questions`, `alphabet` and `quebec-et-francophonie`.
// A CARRY MOVES A POPULATION EVEN WHEN AN AUTHORING DOES NOT, which a1.23
// proved against a1.03's ending statistic and a2.26 and a2.27 met again. The
// neighbouring suites are re-run AFTER this merge, not before.
//
...
import { ALL_ROWS, UNIT, THEME, REPAIR_IDS, LADDER_IDS } from './data/hotel-corpus.ts';
import { LESSON, ITEM_IDS } from './data/hotel-lesson.ts';
...
async function main() {
  ...
  // ['hebergement'] in the spine script, in Postgres and in the seed, because
  ...
  // author-full-curriculum-spine.ts would revert 74 of 75 unit titles, which is
  // exactly what spine-drift.test.ts exists to catch.
  const unit = seed.units.find((u) => u.id === UNIT.id);
  if (!unit) die(`${UNIT.id} is not in the seed`);
  if (!(unit.themes ?? []).includes(THEME)) die(`the seed unit row does not carry the theme ${THEME}; band blocking step 2 has not landed`);
  if ((unit.themes ?? []).includes('voyage')) die("the seed unit row still carries the phantom theme 'voyage'");
  // `voyage` must be at ZERO in the seed: it is a phantom with no themeMeta
  // entry, and the first row authored there would be invisible to the theme hub.
  const voyageRows = seed.items.filter((i) => i.theme === 'voyage').length;
  if (voyageRows) die(`${voyageRows} row(s) carry the phantom theme 'voyage' in the seed`);
  ...
  if ([...REPAIR_IDS].some((id) => !ids.has(id))) die("a2.07's repair rows did not reach the seed, so s13-repair would render empty cards");
  if (LADDER_IDS.some((id) => !ids.has(id))) die('a ladder row did not reach the seed, and the ladder is what three other units cite');
