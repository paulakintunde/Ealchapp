// DELF B2 blanc-05, against the format's rules.
//
...
import * as blanc05 from './paper.ts';

test('delf blanc-05: the schema accepts every task and the paper', () => {
  for (const t of blanc05.TASKS) {
    const issues = validateExamTask(t);
    ok(issues.length === 0, `${t.id}: ${issues.map((i) => `${i.path} ${i.message}`).join(' · ')}`);
  }
  const p = validateExamPaper(blanc05.PAPER);
  ok(p.length === 0, p.map((i) => `${i.path} ${i.message}`).join(' · '));
});

test('delf blanc-05: every rule of the format', () => {
  const problems = delfPaperViolations(blanc05);
  ok(problems.length === 0, `blanc-05 breaks the format:\n  ${problems.join('\n  ')}`);
});
