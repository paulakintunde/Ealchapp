// DELF B2 blanc-03, against the format's rules.
//
...
import * as blanc03 from './paper.ts';

test('delf blanc-03: the schema accepts every task and the paper', () => {
  for (const t of blanc03.TASKS) {
    const issues = validateExamTask(t);
    ok(issues.length === 0, `${t.id}: ${issues.map((i) => `${i.path} ${i.message}`).join(' · ')}`);
  }
  const p = validateExamPaper(blanc03.PAPER);
  ok(p.length === 0, p.map((i) => `${i.path} ${i.message}`).join(' · '));
});

test('delf blanc-03: every rule of the format', () => {
  const problems = delfPaperViolations(blanc03);
  ok(problems.length === 0, `blanc-03 breaks the format:\n  ${problems.join('\n  ')}`);
});
