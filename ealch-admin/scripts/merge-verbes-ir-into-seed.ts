/* Merges a2.10.l1 "Les verbes en -IR" into ealch-v2/src/content/seed.json.
 *
 *     pnpm tsx scripts/merge-verbes-ir-into-seed.ts --dry-run
 *     pnpm tsx scripts/merge-verbes-ir-into-seed.ts
 *
 ...
 *
 * `verbes` is OUTSIDE SEED_CUT.themes: 419 published in Postgres against 65 in
 * the seed, and `verbes-essentiels` is 535 against 25. Eight of this lesson's ten
 * imported rows are not in seed.json today. A lesson whose itemIds resolve to
 * nothing renders empty cards on a device, so this merge CARRIES every imported
 * row. a2.01 hit the same wall with thirty, a2.09 with seventeen, a1.22 with
 * forty. Both halves of the item count are checked separately below, because
 * "the count went up" is not evidence that the right things went up.
 *
...
import {
  CONTRAST_PAIR, DICTATION_IDS, DICTEE_NEAR_MISS, DRILL_ADDITIONS, ENDINGS, GROWING_ENDINGS,
  NOT_THIS_FAMILY, NOT_THIS_FAMILY_UNIT, NUMBER_PAIRS, RESPELL_REPAIRS, SINGULAR_TRIPLES,
...
console.log(`\n  merging a2.10.l1 into seed.json${DRY_RUN ? '  (DRY RUN)' : ''}`);
...
/* ── The four claims that ARE the lesson ─────────────────────────────────── */
{
  const sec = LESSON.sections.find((s) => (s as { id?: string }).id === CONTRAST_SECTION_ID);
  if (!sec || sec.type !== 'tapTable') die(`${CONTRAST_SECTION_ID} is gone or is no longer a tapTable`);
  if (sec.rows.length !== 6) die(`${CONTRAST_SECTION_ID} has ${sec.rows.length} rows, expected 6`);
  ...
  if (Math.abs(iSing - iPlur) !== 1) {
    die(`${CONTRAST_PAIR.singular} and ${CONTRAST_PAIR.plural} are rows ${iSing + 1} and ${iPlur + 1} of ${CONTRAST_SECTION_ID}. They must be neighbours.`);
  }
  if (!sec.rows[iSing].say || !sec.rows[iPlur].say) die(`a contrast row of ${CONTRAST_SECTION_ID} cannot be played with one tap`);
  if (sec.rows.some((r, i) => r.cells[0] !== ENDINGS[i].person)) die(`${CONTRAST_SECTION_ID} is no longer index-aligned with ENDINGS`);
}
for (const triple of SINGULAR_TRIPLES) {
  const tails = triple.map((id) => afterPronoun(VERBES_IR.find((w) => w.id === id)?.respell ?? ''));
...
for (const [sing, plur] of NUMBER_PAIRS) {
  ...
  if ((b.respell ?? '').length <= (a.respell ?? '').length) {
    die(`${plur} is respelled no longer than ${sing}. The plural puts a sound on the end and the card has to show it.`);
  }
}
{
  ...
  const gone = THE_TEN.filter((v) => !learner.includes(v));
  if (gone.length) die(`verb(s) named by no screen: ${gone.join(', ')}`);
  const unnamed = NOT_THIS_FAMILY.filter((v) => !learner.includes(v));
...
/** THE TEN MUST SHOW THE REPAIRED RESPELLING IN THE SEED, not the stored one. The
 *  lesson prints the repaired value on a card; if the seed row still holds the
 *  broken one, the flashcard hub and the lesson disagree on the same word.
 *  `remplir` is the one that matters: the lesson prints it beside
 *  `Je remplis le formulaire.`, whose respelling is `rahⁿ-plee`. */
{
...
/** AND EVERY ONE OF THE TEN MUST CARRY A `flashcard` DRILL IN THE SEED, because
 *  every one is released by a tranche and served as a hub card. Two of the ten
 *  arrive without it. */
{
