// TCF Canada blanc-03, against the format's rules.
//
...
test('tcf blanc-03: the schema accepts every task and the paper', () => {
...
test('tcf blanc-03: each section clock matches the blueprint', () => {
...
test('tcf blanc-03: a perfect paper indexes to exactly the weighted maximum', () => {
...
test('tcf blanc-03: no task id collides with an earlier paper', () => {
  const mine = new Set([...idsOf(CO_TASKS), ...idsOf(CE_TASKS)]);
  for (const [name, co, ce] of EARLIER) {
    for (const id of [...idsOf(co), ...idsOf(ce)]) {
      ok(!mine.has(id), `${id} belongs to both blanc-03 and ${name}`);
    }
  }
});

test('tcf blanc-03: the answer key differs from every earlier paper', () => {
  for (const [name, co, ce] of EARLIER) {
    for (const [ep, mine, theirs] of [
      ['CO', keySeq(CO_TASKS), keySeq(co)],
      ['CE', keySeq(CE_TASKS), keySeq(ce)],
    ] as const) {
      strictEqual(mine.length, theirs.length, `${ep}: blanc-03 and ${name} have different item counts`);
      const same = mine.filter((k, i) => k === theirs[i]).length;
      ok(same < mine.length, `${ep}: blanc-03 has ${name}'s exact key sequence`);
      ok(
...
test('tcf blanc-03: no document is reused from an earlier paper', () => {
  const mine = new Set(textOf([...CO_TASKS, ...CE_TASKS]).map((t) => t.slice(0, 80)));
  for (const [name, co, ce] of EARLIER) {
    for (const t of textOf([...co, ...ce])) {
      ok(!mine.has(t.slice(0, 80)), `a document opens identically in blanc-03 and ${name}: ${t.slice(0, 50)}`);
    }
  }
});

test('tcf blanc-03: every open task carries a rubric and a model answer', () => {
...
test('tcf blanc-03: the interaction task can answer what the document invites', () => {
  ...
  for (const needle of ['garantie', 'diagnostic', 'delai', 'facture', 'refus']) {
    ok(
