/* Regenerates data/pronoms-indirect-rows.gen.ts, the recorded read behind a2.24.
 *
 * EIGHTEEN ROWS IMPORTED AND NOT ONE HEADWORD AUTHORED — corrections §2 for the
 * NINTH build running. Ten infinitives out of five themes, two respelled pronoun
 * phrases, and six published sentences.
 *
 * ── THE MEASUREMENTS THE LESSON RESTS ON, RE-RUN ON EVERY REGENERATION ─────
 *
 * THE FRAME CELLS, AS WHOLE SENTENCES. The brief reports « je lui parle » once
 *   and « je leur parle » once and concludes the construction is almost absent.
 *   The corpus file's header §1 says something narrower and stronger: every BARE
 *   FRAME is absent as a whole sentence, and the CONSTRUCTION is not absent at
 *   all — it is published, in this theme, with this lesson's own teaching notes
 *   on it. Both figures are printed so the difference is on the record.
 * THE THEME HAS NO DUPLICATE `fr`, computed the way flashhub-coverage computes
 *   it. 534 rows and zero duplicate groups; the generator refuses if an authored
 *   frame would collide.
 * THE RESPELL POPULATION for every divergent word, every published row. The
 *   corpus file's repair table is fifteen rows and the brief's list is four; if a
 *   sixteenth appears the author should know before the merge.
 * THE FALSE POSITIVES, through the real function, because §11's claim that
 *   `tay-lay-FOHN` is flagged and `tay-lay-FON` is not is the reason nine cards
 ...
 * A GENDERED row, which would join a1.03's measured ending population. This is
 *   not theoretical here: `écrire` has five published rows and fr.a1.ecole.049
 *   is gendered, so the refusal is what keeps the right one imported.
 * A row inside this build's id block that this build does not own.
 * A row carrying TWO OBJECT PRONOUNS, which is a2.25's or nobody's. The theme
 *   publishes three of them (« Tu le lui donnes tout de suite. » and two more)
 *   and importing one would teach the thing this lesson reserves.
 ...
 *
 *     pnpm tsx scripts/_a224_manifest.ts
 */
import './env';
...
const OUT = join(HERE, 'data', 'pronoms-indirect-rows.gen.ts');
const NL = '\n';

/** The bare frames the lesson authors. The corpus header's §1 claim is that each
 *  is absent as a WHOLE sentence, and the generator refuses on that rather than
 *  on the weaker substring figure. */
const FRAME_CELLS = ROWS.map((r) => r.fr);

/** Every word whose published respellings this build inspected. */
const DIVERGENT = ['répondre', 'demander', 'montrer', 'envoyer', 'parler', 'donner', 'écrire', 'téléphoner'];

/** a2.25's forms, refused on every imported row. `en` as a PREPOSITION is
 *  everywhere in the corpus and is not what this looks for. */
const Y_PRONOUN = /(?<![\p{L}\p{N}'’-])(j'y|n'y|il y va|elle y va|on y va|y aller|vas-y|y consacrent)(?![\p{L}\p{N}'’-])/iu;
const EN_PRONOUN = /(?<![\p{L}\p{N}-])(j'en|n'en|tu en as|il en a|elle en a|on en a|en veux|en prends)(?![\p{L}\p{N}'’-])/iu;

/** TWO OBJECT PRONOUNS IN ONE CLAUSE, which is a2.25's or nobody's. Guard the
 *  THING and not the letters (corrections §14.4): `les` is a French article on
 *  every second row in the corpus, so this requires an actual ADJACENT PAIR from
 *  the two closed sets.
 *
 *  `nous` and `vous` are DELIBERATELY ABSENT from the first set. They are the
 *  two forms that are subject and object with the same spelling, so a version
 *  holding them fires on « Nous leur parlons. », which carries exactly one
 *  pronoun. Kept identical to the batch's copy on purpose. */
const TWO_PRONOUNS =
 ...
 *  using it cannot see `j'ai`, `qu'il` or `d'objet`. Dropped from the LEFT and
 *  kept on the right, which is what §14.3 prescribes. It bites this build by
 *  name: the unit's own `sub` is « Pronoms d'objet indirect ». */
...
async function main() {
  ...
  for (const r of rows) {
    ...
    if (Y_PRONOUN.test(r.fr) || EN_PRONOUN.test(r.fr)) die(`${r.id} « ${r.fr} » carries y or en as a PRONOUN, which is a2.25's lesson`);
    if (TWO_PRONOUNS.test(r.fr)) die(`${r.id} « ${r.fr} » carries two object pronouns in one clause, which is a2.25's or nobody's`);
  }
  ...
  // ── The false positives, through the real function ──────────────────────
  for (const fp of FALSE_POSITIVES) {
    if (!hasPlainNasalFor(fp.fr, fp.flagged)) {
      die(`${fp.fr}: « ${fp.flagged} » is no longer FLAGGED. The corpus file's §11 rests on it being a false positive; `
        + 'if the checker has been fixed, re-measure rather than deleting the entry.');
    }
  ...
  if (nonZeroExact.length) {
    die(`the corpus file claims every bare frame is absent as a whole sentence, and these exist: ${nonZeroExact.map(([k, v]) => `${k} (${v})`).join(', ')}`);
  }

  // ── THE CONSTRUCTION, which is the half the brief got wrong ─────────────
  const CONSTRUCTION = ['je lui', 'je leur', 'il lui', 'elle lui', 'nous leur', 'lui parle', 'leur parle'];
  const construction: Record<string, number> = {};
  for (const p of CONSTRUCTION) {
    construction[p] = sentences.filter((s) => !mine.has(s.id) && s.fr.toLowerCase().includes(p)).length;
  }
  ...
  const body = `// A RECORDED READ OF POSTGRES. Generated by scripts/_a224_manifest.ts.
// DO NOT EDIT BY HAND. Re-run the generator instead.
//
// ${rows.length} rows imported: ${IMPORTED.length} headwords, ${IMPORTED_PHRASES.length} respelled pronoun phrases and
...
//   probe measured and it is a weaker statement. ${new Date().toISOString().slice(0, 10)}:
//
${FRAME_CELLS.map((c2) => `//     ${c2.padEnd(30)} exact ${String(exact[c2]).padStart(3)}   as a substring ${String(substring[c2]).padStart(3)}`).join(NL)}
//
...
//
${CONSTRUCTION.map((p) => `//     ${p.padEnd(14)} ${String(construction[p]).padStart(3)}`).join(NL)}
//
...
`;
  ...
  console.log('    the construction, which the brief calls almost absent:');
  for (const p of CONSTRUCTION) console.log(`      ${p.padEnd(14)} ${String(construction[p]).padStart(3)}`);
  console.log('    respell divergence:');
  for (const w of DIVERGENT) {
    console.log(`      ${w.padEnd(12)} ${flaggedByWord[w]} flagged · ${Object.entries(divergence[w]!).map(([rs, ids]) => `${rs} (${ids.length})`).join(' · ')}`);
  }
  console.log('');
