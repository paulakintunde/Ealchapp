// TCF Canada blanc-05, against the format's rules.
//
...
// valid papers ends up with two non-adjacent ones sharing a key sequence — and
// this is the paper where that matters most, since it closes the pack.
...
import { CO_TASKS as CO4, CE_TASKS as CE4 } from '../tcf-blanc04/paper.ts';
...
test('tcf blanc-05: the schema accepts every task and the paper', () => {
...
test('tcf blanc-05: each section clock matches the blueprint', () => {
...
test('tcf blanc-05: a perfect paper indexes to exactly the weighted maximum', () => {
...
test('tcf blanc-05: no task id collides with an earlier paper', () => {
  const mine = new Set([...idsOf(CO_TASKS), ...idsOf(CE_TASKS)]);
  for (const [name, co, ce] of EARLIER) {
    for (const id of [...idsOf(co), ...idsOf(ce)]) {
      ok(!mine.has(id), `${id} belongs to both blanc-05 and ${name}`);
    }
  }
});

test('tcf blanc-05: the answer key differs from every earlier paper', () => {
  for (const [name, co, ce] of EARLIER) {
    for (const [ep, mine, theirs] of [
      ['CO', keySeq(CO_TASKS), keySeq(co)],
      ['CE', keySeq(CE_TASKS), keySeq(ce)],
    ] as const) {
      strictEqual(mine.length, theirs.length, `${ep}: blanc-05 and ${name} have different item counts`);
      const same = mine.filter((k, i) => k === theirs[i]).length;
      ok(same < mine.length, `${ep}: blanc-05 has ${name}'s exact key sequence`);
      ok(
...
test('tcf blanc-05: no document is reused from an earlier paper', () => {
  const mine = new Set(textOf([...CO_TASKS, ...CE_TASKS]).map((t) => t.slice(0, 80)));
  for (const [name, co, ce] of EARLIER) {
    for (const t of textOf([...co, ...ce])) {
      ok(!mine.has(t.slice(0, 80)), `a document opens identically in blanc-05 and ${name}: ${t.slice(0, 50)}`);
    }
  }
});

test('tcf blanc-05: every open task carries a rubric and a model answer', () => {
...
test('tcf blanc-05: the interaction task can answer what the document invites', () => {
  ...
  //
  // The three disputed line items are checked individually. A bank that could
  // only answer "the quote is binding" would leave a candidate who queries the
  // items one by one talking to the catch-all.
  for (const needle of ['devis', 'preparation', 'deplacement', 'revision', 'recours']) {
    ok(
...
/* ── The pack is now closed ────────────────────────────────────────────────*/

test('tcf: five papers, and no two of them share a single listening document', () => {
  // Stated once, here, because blanc-05 is the last paper and the only place
  // from which all five are visible. Each paper checks itself against its
  // predecessors; nothing until now checked the pack as a whole, and a pack is
  // what a candidate sits.
  const packs: [string, typeof CO_TASKS][] = [
    ['blanc-01', CO1],
    ['blanc-02', CO2],
    ['blanc-03', CO3],
    ['blanc-04', CO4],
  ...
  for (const [name, co] of packs) {
    for (const t of textOf(co)) {
      const head = t.slice(0, 80);
      const owner = seen.get(head);
      ok(!owner, `a listening document is shared by ${owner} and ${name}: ${t.slice(0, 50)}`);
      seen.set(head, name);
    }
  }
  strictEqual(seen.size, 24 * 5, 'five papers should hold 120 distinct listening documents');
});
