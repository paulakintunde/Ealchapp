// TEF Canada blanc-04 — the assembled paper.
//
// Where the authored blocks become a paper: the keys are scattered, the sections
// are ordered, and the scoring tables are attached. Everything downstream reads
// from here, so nothing can validate one shape and write another.
//
// The scoring tables come from `../tef/scoring.ts`, shared with blanc-01 rather
// than copied. Two papers with private copies would eventually disagree, and
// then the same raw score would report different NCLC bands depending on which
// paper a candidate happened to sit.
...
import { CE_A, CE_B, CE_C } from './ce.ts';
import { CE_DE, CE_F, CE_G } from './ce-fg.ts';
