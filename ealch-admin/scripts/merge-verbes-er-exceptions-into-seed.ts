/* Merges a2.09.l1 "Les verbes en -ER : exceptions" into
 * ealch-v2/src/content/seed.json.
 *
 *     pnpm tsx scripts/merge-verbes-er-exceptions-into-seed.ts --dry-run
 *     pnpm tsx scripts/merge-verbes-er-exceptions-into-seed.ts
 *
 ...
 *
 * `verbes` is OUTSIDE SEED_CUT.themes: 393 published in Postgres, 35 in the seed,
 * and most of the themes the seventeen verbs live in are outside it too. A lesson
 * whose itemIds resolve to nothing renders empty cards on a device, so this merge
 * CARRIES every imported row. a2.01 hit the same wall with thirty and a1.22 with
 * forty. Both halves of the item count are checked separately below, because "the
 * count went up" is not evidence that the right things went up.
 *
...
import {
  ACCENT_TAKERS, DICTATION_IDS, DICTEE_NEAR_MISS, DOUBLERS, DRILL_ADDITIONS, MECHANISMS,
...
console.log(`\n  merging a2.09.l1 into seed.json${DRY_RUN ? '  (DRY RUN)' : ''}`);
...
/* ── The four claims that ARE the lesson ─────────────────────────────────── */
{
  ...
  if (!grid || grid.type !== 'cheatSheet') die(`${GRID_SECTION_ID} is gone or is no longer a cheatSheet`);
  const text = strings(grid).join('\n');
  const noReason = PATTERNS.filter((p) => !text.includes(p.reason));
  if (noReason.length) die(`${GRID_SECTION_ID} no longer carries the reason for ${noReason.map((p) => p.label).join(', ')}. The reason IS the lesson.`);
  if (MECHANISMS.length !== 2) die(`PATTERNS now declares ${MECHANISMS.length} mechanisms and every screen says two`);
}
{
  const pair = LESSON.sections.find((s) => (s as { id?: string }).id === SPLIT_SECTION_ID);
  if (!pair || pair.type !== 'tapTable') die(`${SPLIT_SECTION_ID} is gone or is no longer a tapTable`);
  if (pair.cols[1] !== SPLIT_COLUMNS.doubles || pair.cols[2] !== SPLIT_COLUMNS.accents) {
    die(`${SPLIT_SECTION_ID} no longer compares ${SPLIT_COLUMNS.doubles} against ${SPLIT_COLUMNS.accents} in two columns. Separating them destroys the teaching.`);
  }
  if (pair.rows.length !== 6) die(`${SPLIT_SECTION_ID} has ${pair.rows.length} rows, expected 6`);
}
{
  ...
  for (const v of [...DOUBLERS, ...ACCENT_TAKERS]) {
    if (!learner.includes(v)) die(`${v} is on the memorised list and named by no screen`);
  }
}
for (const p of SOFT_STEM_SOUND) {
  ...
  if (!(a?.respell ?? '').toLowerCase().includes(sound) || !(b?.respell ?? '').toLowerCase().includes(sound)) {
    die(`${p.still} and ${p.moving} must both carry "${p.sound}". The letter exists so the sound does NOT change.`);
  }
}
...
/** THE SEVENTEEN MUST SHOW THE REPAIRED RESPELLING IN THE SEED, not the stored
 *  one. The lesson prints the repaired value on a card; if the seed row still
 *  holds the broken one, the flashcard hub and the lesson disagree on the same
 *  word. `commencer` is the one that matters: the lesson prints it beside
 *  `nous commençons`. */
{
