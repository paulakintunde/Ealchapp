...
export const ALREADY_YOURS =
  `${NEGATION_UNIT} gave you the two halves and where they go. ${ALLER_UNIT} gave you the six forms of aller. This lesson is those two facts standing next to each other, and the only new thing is which of two verbs the halves go round.`;
...
export const FUTUR_PROCHE_TERMS: Record<string, LessonTerm> = {
  whichVerb: {
    ...
    body:
      `${OWNS_CLAIM} ${A118_REFRAME} is what ${NEGATION_UNIT} told you, and it was enough while there was only one verb in the sentence to wrap. Now there are two, and only one of them changed when you picked the person you are talking about. That is the one. The other is sitting there in its naming form and it has not moved at all, so there is nothing about it for the two halves to go round.`,
    examples: [
  ...
  neverChanges: {
    ...
    body:
      `${A213_REFRAME} That is ${MODAL_UNIT}'s line, about pouvoir and vouloir and devoir, and it is exactly as true here. ${ALLER_UNIT} said the same thing about what follows de and called it ${A202_NAMING_FORM}. Whatever you put behind aller goes in untouched: it does not agree with anybody, it does not take an ending, and it is the same word in all six rows of the table.`,
    examples: [
      { itemId: 'fr.a2.verbes.347', note: `${MODAL_UNIT}'s card. Payer, behind a modal.` },
  ...
  goingTo: {
    ...
    examples: [
      ...
      { itemId: 'fr.a2.verbes.519', note: `And with a time on the end, which is ${TIME_UNIT}'s phrase, unchanged.` },
    ],
  },
  ...
  timeWord: {
    ...
    body:
      `« Je vais manger » on its own can be a plan or it can be somebody walking towards a kitchen. Put a time on it and there is nothing left to work out. ${TIME_UNIT} gave you dans plus a length one lesson ago and taught it with a present-tense verb, which is correct on its own; ${DANS_PAIR.why}`,
    examples: [
      { itemId: 'fr.a2.prepositions-essentielles.172', note: `${TIME_UNIT}'s sentence. A present tense about something ahead.` },
  ...
  sameShape: {
    ...
    body:
      `${POSITION_CLAIM} You have already seen it once, behind ${MODAL_UNIT}'s three verbs: « je ne peux pas venir » puts the two halves round peux and leaves venir alone. You will see it again in the very next lesson, on a past tense, and it behaves the same way there. One rule, three places, and the third one is ${PAST_UNIT}.`,
    examples: [
...
export const HANDOVER = {
  [PAST_UNIT]: 'The negation rule is the one this lesson states and a2.05 extends it to the auxiliary: the two halves go round the verb that changed, and the participle stays outside them exactly as the naming form does here. The wording to match is the reframe.',
