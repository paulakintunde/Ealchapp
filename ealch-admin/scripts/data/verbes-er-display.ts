...
//
// The thirty are imported rows, so their respellings live in Postgres and not in
// this repo. Seven of them break the house nasal rule and are repaired by this
// build. That creates a trap worth naming: the RECORDED value in
// verbes-er-imported.ts is what the database held BEFORE the batch ran, and the
// screen must show what it holds AFTER. A card built straight off the manifest
// would print `ahn-TRAY` for `entrer` on a device while Postgres held `ahⁿ-TRAY`,
// and the lesson's own nasal assertions would then fail against its own content.
//
...
import { RESPELL_REPAIRS } from './verbes-er-corpus.ts';
import { VERB_BY_NAME } from './verbes-er-imported.ts';

/** id -> the respelling this build writes. Built from the repair table so the
 ...
 *  and `vocabThemes` cards carry no itemId at all, which is why the thirty are
