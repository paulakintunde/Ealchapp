// a2.28.l1 « Chez le médecin » — merge into ealch-v2/src/content/seed.json.
//
//   pnpm tsx scripts/merge-medecin-into-seed.ts
//
// THE SEED IS A CUT, AND THIS UNIT SITS ON THE WIDEST DIVERGENCE IN THE BAND.
// Before this merge, `symptomes` holds 334 published rows in Postgres and ZERO
// in the seed, and `systeme-de-sante` holds 583 and zero. So this script does
// NOT merely add the authored rows. It pulls every row the lesson REFERENCES
// out of Postgres and writes those too, or the imported cards render empty on
// device while the tests pass against a corpus that has them.
//
// THAT DIVERGENCE IS THE BUILD'S HEADLINE. The design measured the health
// vocabulary on the seed, saw nothing, and planned 75 to 90 authored rows. The
// symptom lexicon was already finished: 193 published a2 rows in `symptomes`
// alone. This unit authored 34 and imported the rest.
//
// THIS MERGE CARRIES ROWS INTO THEMES THIS UNIT DOES NOT OWN: `metiers`,
// `systeme-de-sante` and `au-restaurant` (a2.07's frozen repair block).
// A CARRY MOVES A POPULATION EVEN WHEN AN AUTHORING DOES NOT, which a1.23
// proved against a1.03's ending statistic and a2.27 met again. The neighbouring
// suites are re-run after this merge, not before.
//
...
import { ALL_ROWS, UNIT, THEME, BODY_THEME, REPAIR_IDS } from './data/medecin-corpus.ts';
import { LESSON, ITEM_IDS } from './data/medecin-lesson.ts';
...
const MY_THEMES = [THEME, BODY_THEME];
...
async function main() {
  ...
  // ['symptomes', 'corps'] in the spine, in Postgres and in the seed, because
  // band blocking step 2 landed before this build started.
  ...
  if ((unit.themes ?? []).includes('sante')) die("the seed unit row still carries the phantom theme 'sante'");
  // `sante` must be at ZERO in the seed too: a1-24-corps.test.ts's assertion is
  // global over the whole file, not scoped to a1.24.
  const santeRows = seed.items.filter((i) => i.theme === 'sante').length;
  if (santeRows) die(`${santeRows} row(s) carry the phantom theme 'sante' in the seed; a1-24-corps.test.ts pins it at zero`);
  ...
  if (six.some((x) => !x)) die("a2.07's repair rows did not reach the seed, so s09-repair would render empty cards");
