 ...
 *
 * Writes a2.14.l1 and its thirty authored rows into seed.json with NO GUARDS AT
...
import { SAVOIR_CONNAITRE, toItem } from './data/savoir-connaitre-corpus.ts';
import { SAVOIR_CONNAITRE_LESSON } from './data/savoir-connaitre-lesson.ts';
...
for (const it of SAVOIR_CONNAITRE.map(toItem)) byId.set(it.id, it);

const out: Seed = {
  ...seed,
  items: [...byId.values()],
  lessons: [...seed.lessons.filter((l) => l.id !== SAVOIR_CONNAITRE_LESSON.id), SAVOIR_CONNAITRE_LESSON],
  units: seed.units.map((u) => (u.id === 'a2.14'
    ? { ...u, lessonIds: [...new Set([...(u.lessonIds ?? []), SAVOIR_CONNAITRE_LESSON.id])] }
    : u)),
};

writeFileSync(SEED, `${JSON.stringify(out, null, 2)}\n`, 'utf8');
console.log(`  FORCED ${SAVOIR_CONNAITRE_LESSON.id} into the seed with no guards. ${out.items.length} items, ${out.lessons.length} lessons.`);
