...
import { BILAN_LESSON, BILAN_DICTATION_IDS } from './data/bilan-lesson.ts';
import { REVIEW, KIT_IMPORTED, KIT_REUSED } from './data/bilan-imported.ts';
import { AUTHORED_ITEMS, DRILL_ADDITIONS } from './data/bilan-corpus.ts';
import { REFRAME } from './data/bilan-terms.ts';

const L = BILAN_LESSON;
let bad = 0;
...
for (const s of L.sections) {
  ...
  if (s.type === 'trapDrill') {
    for (const [k, q] of (s.drill ?? []).entries()) inMission.push({ section: sid, q: `drill ${k + 1}`, correct: q.correct, opts: q.opts });
  }
  if (s.type === 'listening') {
...
const known = new Set([...REVIEW.map((r) => r.id), ...KIT_IMPORTED.map((r) => r.id), ...KIT_REUSED.map((r) => r.id), ...AUTHORED_ITEMS.map((a) => a.id)]);
...
for (const id of BILAN_DICTATION_IDS) {
  const r = [...KIT_IMPORTED, ...KIT_REUSED, ...REVIEW].find((x) => x.id === id);
  if (!r) { fail(`dictation target ${id} is not in the manifest`); continue; }
  // The batch ADDS `dictation` to three kit rows that never had it, so a target
  // is fine if it already carries the drill OR if DRILL_ADDITIONS will give it one.
  const willHave = DRILL_ADDITIONS.some((d) => d.id === id && d.add === 'dictation');
  const hasNow = 'drills' in r && (r.drills ?? []).includes('dictation');
  if (!willHave && !hasNow) fail(`dictation target ${id} carries no dictation drill and the batch does not add one`);
