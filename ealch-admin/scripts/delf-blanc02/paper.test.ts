// DELF B2 blanc-02, against the format's rules.
//
...
import * as blanc02 from './paper.ts';

test('delf blanc-02: the schema accepts every task and the paper', () => {
  for (const t of blanc02.TASKS) {
    const issues = validateExamTask(t);
    ok(issues.length === 0, `${t.id}: ${issues.map((i) => `${i.path} ${i.message}`).join(' · ')}`);
  }
  const p = validateExamPaper(blanc02.PAPER);
  ok(p.length === 0, p.map((i) => `${i.path} ${i.message}`).join(' · '));
});

test('delf blanc-02: every rule of the format', () => {
  const problems = delfPaperViolations(blanc02);
  ok(problems.length === 0, `blanc-02 breaks the format:\n  ${problems.join('\n  ')}`);
});
