...
/**
 * One category's Voice Flash sub-theme list: a one-column stack of compact
 * cards, one per catalogue theme under this domain that actually holds a
 * voiceflash item, each opening that sub-theme's own spoken drill. Same shape
 * as flashthemes.tsx (minus the by-card-type picker — voiceflash has no
 * card types, every item is the same spoken prompt/response pair).
 *
 * The stack is difficulty-ordered (a sub-theme's floor band, LEVELS order),
 * so the column itself reads as a ramp. Only sub-themes with a nonzero count
 * render — an empty theme is a catalogue row, not a deck.
 */
export default function VoiceThemes() {
  const t = useTheme();
  const T = useT();
  ...
  const rows = useMemo(() => {
    ...
    return (corpus.themes ?? [])
      ...
      .map((th) => ({
        ...
        lo: th.levelRange[0],
        hi: th.levelRange[1],
        count: selectItems(corpus, 'voiceflash', { theme: th.slug }).length,
      }))
      .filter((r) => r.count > 0)
  ...
  return (
    <View style={{ flex: 1, backgroundColor: t.bg, paddingTop: insets.top }}>
      ...
      />
      <FocusHeader onClose={() => router.back()} onSettings={() => router.push('/settings')} title={T.vfTitle} />
      ...
      >
        ...
        <View style={{ gap: 10 }}>
          {rows.map((r) => (
            <Press
              key={r.slug}
              onPress={() => router.push({ pathname: '/voiceflash', params: { theme: r.slug } })}
              scale={0.99}
              style={{
                minHeight: 76,
                paddingVertical: 10,
