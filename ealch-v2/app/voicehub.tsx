...
/**
 * The themed Voice Flash hub: every catalogue domain that actually holds a
 * voiceflash item, as a compact tappable category card, two per row. A card
 * opens the category's sub-theme list (/voicethemes), which opens the drill
 * itself (/voiceflash). Same shape as flashhub.tsx — the grid is a view over
 * the corpus catalogue, not a hardcoded list, so a domain with no voice items
 * yet simply doesn't render (a dead door teaches distrust).
 */
export default function VoiceHub() {
  ...
  const cats = useMemo(() => {
    ...
    return domains
      .map((d) => ({
        ...
        count: selectItems(corpus, 'voiceflash', { themes: themesOf.get(d.slug) ?? [] }).length,
      }))
      .filter((c) => c.count > 0);
  }, [corpus]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: t.bg, paddingTop: insets.top }}>
      ...
      />
      <FocusHeader onClose={() => router.replace('/home')} onSettings={() => router.push('/settings')} title={T.vfTitle} />
      ...
      >
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}>
          {cats.map(({ slug, meta, count }) => (
            <Press
              key={slug}
              onPress={() => router.push({ pathname: '/voicethemes', params: { domain: slug } })}
              scale={0.98}
              style={{
                width: '47.5%',
                minHeight: 132,
