...
- [ ] Fix known bugs blocking normal use: audio doesn't pause on backgrounding, exam response lost on process death, dark mode toggle ignored, STT continuous-mode regression risk (notification body placeholders were fixed in Phase 6, see Validated). Two further bugs (BUG-04, BUG-05, Phase 23) surfaced live during Phase 7's own BUG-02 device verification (2026-09-22): recording-based exam tasks never show a restored "already answered" summary after a crash even though the answer data survives, and `submit()`/`sectionStatusFor()` can silently disagree about which tasks belong to a section, misreporting a genuinely-completed section as "not sat."
