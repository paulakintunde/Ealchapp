...
| 05-07-T2 | 07 | 4 | PAY-02, PAY-05 | T-05-34, T-05-35, T-05-37 | Render-level behaviour confirmed on device, including the over-blocking false-positive check | manual (blocking checkpoint) | `npm --prefix ealch-v2 test && grep -n "const DEV_UNLOCK_A2 = false" ealch-v2/src/store/entitlement.logic.ts` | n/a | awaiting-device |
...
**Note on 05-07-T2's `awaiting-device` status:** the three automated preconditions in this row's Automated Command all passed as of this update: `npm --prefix ealch-v2 test` is 5391/5391 green, and `grep -n "const DEV_UNLOCK_A2 = false" ealch-v2/src/store/entitlement.logic.ts` matches line 242. `adb devices` and the 21-item checklist itself are Task 2's own blocking checkpoint and have not run yet in this execution pass. This row is deliberately NOT marked `green`, because doing so before the device pass actually happens would be a false record of render-level verification that did not occur. Every other row (16 of 17) reflects a real, just-executed automated result, so `nyquist_compliant` stays `true`. The Approval line below names this row as the one item still blocking full phase closure.
...
**Approval:** executed (2026-09-20). 16 of 17 rows in the Per-Task Verification Map are `green`, filled in by 05-07-T1 from the post-phase `npm --prefix ealch-v2 test` run (5391/5391 passing) and `npm --prefix ealch-v2 run typecheck` (clean). Row `05-07-T2` is `awaiting-device`: its automated preconditions passed but the 21-item manual device pass is 05-07's own Task 2, a blocking checkpoint not yet run as of this update. Phase closure is not final until that row resolves to `green` (approved) or the failing items are recorded and a gap-closure plan is written.
